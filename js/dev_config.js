$(document).ready(function(){
	...
	$(document).on('click', '.mode_sel', function(){
		...
		bootbox.confirm("Realmente queres cambiar la configuracion del dispositivo?", function(result) {
			if(result){
			     // AJAX Request
			     $.ajax({
			       url: 'dev_config.php',
			       ...
			       success: function(response){
			          	...
			          	if(response == 1){
			          		// bootbox.alert('<p class="alert alert-success">El modo del dispositivo fue actualizado</p>');
			              	$(el).closest('tr').css('background','#5cb85c');
					        $(el).closest('tr').fadeOut(300,function(){
					              $(this).show();
					        });
